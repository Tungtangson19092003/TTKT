...
/***********************************************************************************************************************
* Function Name: R_UART0_Snooze
* Description  : This function set snooze mode.
                 Only UART0 or UART2 can able to use snooze mode.
* Arguments    : None
* Return Value : status -
*                    MD_OK or MD_BUSY1
***********************************************************************************************************************/
MD_STATUS R_UART0_Snooze(void)
{
    MD_STATUS status = MD_BUSY1;

    /* check if already received data or occurred error, or uart is active */
    if(((SSR01L & 0x60) == 0) && (SRIF0 == 0) && (SREIF0 == 0))
    {
        /* set snooze mode */
        ST0L  = 0x03;
        SSC0L = 0x00;
...
*                    MD_OK or MD_BUSY1
***********************************************************************************************************************/
MD_STATUS R_UART2_Snooze(void)
{
    MD_STATUS status = MD_BUSY1;

    /* check if already received data or occurred error, or uart is active */
    if(((SSR11L & 0x60) == 0) && (SRIF2 == 0) && (SREIF2 == 0))
    {
        /* set snooze mode */
        ST1L  = 0x03;
        SSC1L = 0x00;
