...
extern uint8_t data_received[2];
    ...
    static RBLE_BROADCAST_ENABLE_PARAM  param = {
        ...
        RBLE_GAP_UND_CONNECTABLE ,
        {
            ...
            },
            {
                ...
                /* AdvData        */
                2,  0x01, 0x06,                                                     			   /* Flags               */ 
                23, 0x09, 'R', 'e', 'n', 'e', 's', 'a', 's', ' ', 'B', 'l', 'o', 'o', 'd', ' ',    /* Complete local name */
                'P', 'r', 'e', 's', 's', 'u', 'r', 'e',
                0x00, 0x00, 0x00, 0x00
            },
            {
...
static RBLE_STATUS APP_SCPS_Send_Notify_Command( void )
{
	...
	notify_info.data[0] = g_HighPressureData; 		/* Systolic pressure  */
    notify_info.data[1] = g_LowPressureData;		/* Diastolic pressure  */
    notify_info.data[2] = g_PulseData;				/* Pulse */
	notify_info.data[3] = g_BatteryLevel;           /* Battery level */
...
static void APP_SCPS_CallBack( RBLE_SCPS_EVENT *event )
{
    switch( event->type){
        ...
        case RBLE_SCP_EVENT_SERVER_CHG_CHAR_IND:
			/* Add for blood pressure monitor*/
			if( ( (data_received[0] == 0xff) || (data_received[0] == 0xc5) )&& ( (data_received[1] == 0x55) ||( data_received[1] == 0x3d) ) )
			{
				data_received[0] = 0;
				data_received[1] = 0;
...
static BOOL APP_SCPS_Change_NtfInd_Config_CallBack( RBLE_SCPS_EVENT *event )
{
    if (event->param.cfg_indntf.char_code == RBLE_SCP_SCS_NTF_CFG)
    {
        if(event->param.cfg_indntf.cfg_val & RBLE_SCP_START_NTF)
        {
            ...
            RBLE_Set_Timer_Id( _RBLE_TIMER_ID_APP, APP_Send_Data, 50 );
        }
        else
        {
            /* Disable timer for Notification trigger */
